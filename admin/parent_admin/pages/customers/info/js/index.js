...
/*
   $.ajax({
              ...
              success: function(result) {
                document.getElementById("data_transaction").innerHTML = result;
                //  var content_info = JSON.parse(result);
                 // alert(content_info.amount);
                 // alert(content_info.due_date);
                 // alert(content_info.confirmation);
...
$('#pagination').on('click', 'a', function(e) { // When click on a 'a' element of the pagination div
    ...
    var data = {page: page, per_page: 1,bid: bid}; // Create JSON which will be sent via Ajax
...
$('#pagination').on('click', 'a', function(e) { // When click on a 'a' element of the pagination div
    ...
    var item = $("#id").val();
    $('#data_customer').html('<img src="design/loader-small.gif" alt="" />'); // Display a processing icon
    var data = {page: page, per_page: 10,item: item}; // Create JSON which will be sent via Ajax
        // We set up the per_page var at 4. You may change to any number you need.
        //
     $.ajax({ // jQuery Ajax
        type: 'POST',
